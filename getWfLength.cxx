...
if(argc < 2){ cerr<<"Insufficient arguments. Usage: 1.Data ROOT file 2. Pedestal File\n"; return -1; }
...
double beginTimeByChannel[16]={0.}; // start time of each waveform
int wfNBins[16] = {0}; //number of bins per waveform, before entering evProcessTools::getWindowedAndPaddedEqualBeginGraph()
/*
int *maxPix;
...
//*************APPLYING DELAYS. CODE FROM T. MEURES*****************
   ...
   {//Loop the 16 channels
      ...
      cout<<"Raw waveform "<< a <<" number of samples: "<<gr_v_temp[a]->GetN()<<endl;
...
*/
   ...
   /* Replace grInt with random voltage waveform to check the power scaling with waveform
    * length.
    */
