...
/* Variables used in A3 pre-filter event list */
vector<int> calpuserEventList;
vector<int> surfaceEventList;
int type;
...
int surfaceFilteredEventCount, calpulserFilteredEventCount, snrCutEventCount;
surfaceFilteredEventCount = calpulserFilteredEventCount = snrCutEventCount = 0;
 ...
 */
   ...
   if(rawAtriEvPtr->stationId==3){

      //Preparing list of calpulser and surface events
      ifstream calpulserEventListFile("/data/user/mlu27/analysis/ARA03_analysis_event_lists/ARA03_run"+runNum+"_calpulserEventList.csv");
      if(getEventList(calpulserEventListFile, calpulserEventList)<0){
         cerr<<"Calpulser event list not OK. Aborting..."<<endl;
         return -1;
      }
      calpulserEventListFile.close()

      ifstream surfaceEventListFile("/data/user/mlu27/analysis/ARA03_analysis_event_lists/ARA03_run"+runNum+"_surfaceEventList.csv");
      if(getEventList(surfaceEventListFile, surfaceEventList)<0){
         cerr<<"Surface event list not OK. Aboring..."<<endl;
         return -1;
      }
      surfaceEventListFile.close()
...
*/
   ...
   /* Calpulser Event List Filter */

   if (isInEventList(calpulserEventList, realAtriEvPtr->eventNumber)){
   ...
   /* Surface Event List Filter */
   if (isInEventList(surfaceEventList,realAtriEvPtr->eventNumber)){
