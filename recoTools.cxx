 ...
 */
   ...
   else if (kernel_name=="computeIterativeNormalizedCoherence") clEnv->computeIterativeNormalizedCoherence =clEnv->kernels[i];
...
*/ //cvs.cd(1);
  ...
  //cout<<"env Peak Bin: "<<peakBin<<" Peak Value: "<<y<<endl;
   ...
   //sprintf(envelopename, "xCorrEnvelope_chan%d_%d.C", ant1, ant2);
   //cvs.cd();
   //envelope->Draw("AL");
   //envelope->SetLineColor(kRed);
   //xCorrGraph->Draw("AL");
...
cout<<"Entered reconstruct3DXCorrEnvelopeGetMaxPixAndMapData_2ndRayReco method\n";
int nSamp;
//int nAnt = (int)cleanEvent.size()/2; // Divide by 2 for only one polarization
int nAnt;// = (int)cleanEvent.size();
string pol = string(settings->recoPolType);
...
summary->setTopMaxPix2Info(topMaxPixIdx, topMaxPixCoherence);
...
clEnqueueReadBuffer(clEnv->queue, maxPixCoherenceEachLayerBuffer, CL_TRUE, 0, sizeof(float)*nLayer, maxPixCoherenceEachLayer, 0, NULL, NULL);
err = clFinish(clEnv->queue);
summary->setMaxPix2InfoEachLayer(settings, maxPixIdxEachLayer, maxPixCoherenceEachLayer);
...
maxPixIdx = rank[0];
cout<<"max: "<<M[rank[0]]<<endl;
summary->setMaxPix2Info(rank[0], M[rank[0]]);
...
int nSideExp = settings->nSideExp;
int nDir   = /*summary->onion*/12 * pow(2,nSideExp) * pow(2,nSideExp);;
int nLayer = /*summary->onion*//*settings->nLayer*/1;
printf("Constant N filter nDir: %d nLayer: %d\n",nDir,nLayer);
/*
 * Loading voltsFlat array
 */
...
cout<<"Entered reconstruct3DXCorrEnvelopeGetMaxPixAndMapData_overThresChanConstantNFilter method\n";
int nSamp;
//int nAnt = (int)cleanEvent.size()/2; // Divide by 2 for only one polarization
int nAnt;// = (int)cleanEvent.size();
string pol = string(settings->recoPolType);
...
maxPixIdx = rank[0];
cout<<"max: "<<M[rank[0]]<<endl;
summary->setOverThresChanConstantNMaxPixInfo(rank[0], M[rank[0]]);
...
int nDir   = /*summary->onion*/12 * pow(2,nSideExp) * pow(2,nSideExp);;
int nLayer = /*summary->onion*//*settings->nLayer*/1;
printf("Calpulser filter nDir: %d nLayer: %d\n",nDir,nLayer);
/*
 * Loading voltsFlat array
 */
...
Healpix_Onion *onion = new Healpix_Onion(nSideExp, nLayer, settings->layerFirstRadius, settings->layerLastRadius);
...
//for(int layer=0; layer<nLayer; layer++){
   ...
   //for(int pix=0; pix<nDir; pix++){
      ...
      */
      ...
      //if( k<8 || k>11 )
            solvedRefracDelay.push_back(tempRefracDelay);
      if( tempRefracDelay > 1e9 ) cout<<"Unreasonbaly large delay\n";

      //recoDelays[layer*nDir*nAnt + pix*nAnt + k] = tempDelay;
      recoRefracDelays[k] = tempRefracDelay;
...
int compute3DRecoAnglesWithRadioSplineForSinglePoint(const int nAnt, const float zCenter, const vector<vector<double> >& antLoc,
                                     //const float radius, const int nSideExp,
                                     //Healpix_Onion *onion,
                                     float *recoLauAngles, /*float *recoLauAngles_V, float *recoLauAngles_H,*/
                                     float *recoRecAngles, /*float *recoRecAngles_V, float *recoRecAngles_H,*/
...
void getChannelTotalPowerSNR(const vector<TGraph *>& cleanEvent, int nIntSamp_V, int nIntSamp_H, float *snrArray){
   ...
   for ( int ch=0; ch<(int)cleanEvent.size(); ch++){
         ...
         v2Gr = evProcessTools::getSqrtVoltageSquaredSummedWaveform(cleanEvent[ch], (ch<8?nIntSamp_V:nIntSamp_H));
         int MaxBin = FFTtools::getPeakBin(v2Gr) + (ch<8?nIntSamp_V:nIntSamp_H)/2;
...
int record3DDiffGetFlag_2ndRayReco(recoSettings *settings, recoData *summary, TH1F *dZenDist, TH1F *dAziDist, TH2F *recoTrueZenDist, TH2F *recoTrueAziDist){
   ...
   cout<<"*********** record3DDiffGetFlag_2ndRayReco ***********\n";
   ...
   cout<<"maxPixIdx: "<<summary->maxPixIdx<<" maxPixIdx on the skymap: "<<summary->maxPixIdx%nDir<<endl;
   cout<<"maxPixIdx2: "<<summary->maxPixIdx2<<" maxPixIdx2 on the skymap: "<<summary->maxPixIdx2%nDir<<endl;
   int mpi = (summary->maxPixCoherence>=summary->maxPixCoherence2) ? summary->maxPixIdx : summary->maxPixIdx2;

   pt = hpBase.pix2ang( /*summary->maxPixIdx*/mpi % nDir );
   //float dZen = (pt.theta - zen_true) * 180.f / M_PI;
   //float dAzi = (pt.phi   - azi_true) * 180.f / M_PI;
   ...
   //double recAzi = pt.phi   * 180.f / M_PI;
   summary->setRecoDir(recZen, recAzi);
   summary->setRecoRadius( /*summary->onion->*/onion.getLayerRadius(/*summary->maxPixIdx*/mpi) );
...
int recordConstantNDir(recoSettings *settings, recoData *summary){
   ...
   cout<<"constant N maxPixIdx: "<<summary->constantNMaxPixIdx<<" idx on the skymap: "<<summary->constantNMaxPixIdx%nDir<<endl;
   if( summary->constantNMaxPixIdx != summary->constantNMaxPixIdx%nDir){
      cerr<<"Constant N healpix onion has more than 1 layer !"<<endl;
      return -1;
   }
   pt = hpBase.pix2ang( summary->constantNMaxPixIdx );
   //float dZen = (pt.theta - zen_true) * 180.f / M_PI;
   //float dAzi = (pt.phi   - azi_true) * 180.f / M_PI;

   double zen = pt.theta * 180.f / M_PI;
   double azi = pt.phi   * 180.f / M_PI;
   //float dZen = recZen - summary->trueZen;
   //float dAzi = recAzi - summary->trueAzi;
   ...
   //double recZen = pt.theta * 180.f / M_PI;
   //double recAzi = pt.phi   * 180.f / M_PI;
   summary->setConstantNDir(zen, azi);
...
int recordOverThresChanConstantNDir(recoSettings *settings, recoData *summary){
   ...
   cout<<"*********** recordOverThresChanConstantNDir ***********\n";
   ...
   cout<<"over-threshold chan constant N maxPixIdx: "<<summary->overThresChanConstantNMaxPixIdx<<" idx on the skymap: "<<summary->overThresChanConstantNMaxPixIdx%nDir<<endl;
   if( summary->overThresChanConstantNMaxPixIdx != summary->overThresChanConstantNMaxPixIdx%nDir){
      cerr<<"Over-threshold constant N healpix onion has more than 1 layer !"<<endl;
      return -1;
   }
   pt = hpBase.pix2ang( summary->overThresChanConstantNMaxPixIdx );
   //float dZen = (pt.theta - zen_true) * 180.f / M_PI;
   //float dAzi = (pt.phi   - azi_true) * 180.f / M_PI;
   ...
   //double recZen = pt.theta * 180.f / M_PI;
   //double recAzi = pt.phi   * 180.f / M_PI;
   summary->setOverThresChanConstantNDir(zen, azi);
...
TGraph * impulsivityMeasure(TGraph * wf, double *impulsivity/*TGraph * distance_cdf*/,int pt/*, bool    hilbert*/)
{
  ...
  //if (distance_cdf) distance_cdf->Set(N);
  TGraph *distance_cdf = new TGraph();
  distance_cdf->Set(N);
...
TGraph *truncateCWFreq(TGraph *gr, double cwFreq, double fftRes){
   ...
   for(int i=0; i<nPoint; i++){
      gr->GetPoint(i,f,p);
      if( fabs(f-cwFreq)<fftRes/2.+1e-6 ) grTrunc->SetPoint(i,f,-1000);
