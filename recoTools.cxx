...
for(int baseline=0; baseline<nBaseline; baseline++){
   ...
   /* Let only baselines that consist of antennas on the same string (vertical baselines) have nonzero values */

   if( (abs(ant1-ant2)%4 == 0) && (ant1!=ant2) ){
...
clSetKernelArg(clEnv->computeNormalizedCoherence, 2, sizeof(int),    &nBaseline);
*/
clSetKernelArg(clEnv->computeNormalizedCoherence_onlyVerticalBaselines, 0, sizeof(cl_mem), &MBuffer);
clSetKernelArg(clEnv->computeNormalizedCoherence_onlyVerticalBaselines, 1, sizeof(cl_mem), &CijBuffer);
clSetKernelArg(clEnv->computeNormalizedCoherence_onlyVerticalBaselines, 2, sizeof(int),    &nBaseline);
clSetKernelArg(clEnv->computeNormalizedCoherence_onlyVerticalBaselines, 3, sizeof(int),    &nVerticalBaseline);
workDim = 2;
size_t MGlobalWorkSize[2] = {(size_t)nLayer, (size_t)nDir};

//clEnqueueNDRangeKernel(clEnv->queue, clEnv->computeCoherence, workDim, NULL, MGlobalWorkSize, NULL, 0, NULL, NULL);
clEnqueueNDRangeKernel(clEnv->queue, clEnv->computeNormalizedCoherence_onlyVerticalBaselines, workDim, NULL, MGlobalWorkSize, NULL, 0, NULL, NULL);
clFinish(clEnv->queue);
clEnqueueReadBuffer(clEnv->queue, MBuffer, CL_TRUE, 0, sizeof(float)*nLayer*nDir, M, 0, NULL, NULL);

cout<<"Done computeNormalizedCoherence_onlyVerticalBaselines\n";
...
double zCenter = /*stationCenter[2]*/0.;
