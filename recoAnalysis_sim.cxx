...
double nCut1p5W2, nCut3W2, nCut3p5W2, nCut4W2, nCut6p5W2;
...
//
   ...
   */
   ...
   if ( isCW ){
      ...
      //impulsivityHist_3rd->Fill(impulsivity[index[2]], dummyData->weight);
      //impulsivityHist_avg->Fill(avgImpulsivity, dummyData->weight);
      //_vnchnl3NoMasking_noMaskSat_flattenSat_snrMode1_coherenceThermalCut_snrCut_signalEfficiency.csv<<avgImpulsivity<<",";
...
sprintf(filename, "%s Config %d log(E/eV)=%s Pass-All-Cut Events;sin(#theta_{reco});Event Count", STATION.c_str(), type, ENERGY.c_str());
...
sprintf(filename, "%s Config %d log(E/eV)=%s Pass-All-Cut Events;sin(#theta_{reco});Cumulative Fraction", STATION.c_str(), type, ENERGY.c_str() );
constantNZen_passAllCuts_cumu->SetTitle(filename);
constantNZen_passAllCuts_cumu->GetYaxis()->SetTitleOffset(1.3);
double angles = -20;
