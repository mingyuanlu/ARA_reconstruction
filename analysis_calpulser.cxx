...
int numCalAnt = (int)pulserLocation.size();
for(int a=0; a<numCalAnt; a++) cout<<"cal ant "<<a<<" x: "<<pulserLocation[a][0]<<" y: "<<pulserLocation[a][1]<<" z: "<<pulserLocation[a][2]<<endl;
...
//A2 D6BV
//double cal_zen = 80.4426;;
//double cal_azi = 63.3316;
//double calLocation[3] = {cal_r, cal_zen, cal_azi};
double calLocation[3] = {pulserLocation[3][0], pulserLocation[3][1], pulserLocation[3][2]};
cout<<"pulsertLocation 0: "<<pulserLocation[1][0]<<" 1: "<<pulserLocation[1][1]<<" 2: "<<pulserLocation[1][2]<<endl;
...
compute3DRecoDelaysWithRadioSplineForSinglePoint_cartesianCoord(nAnt, -1.f*stationCenterDepth, antLocation, calRecoDelays, calRecoDelays_V, calRecoDelays_H, calLocation);
...
cvs.Divide(8,8);
TLine *dtLine[64];
double dCalRecoDelays;
for(int b=0; b<64; b++){
   cvs.cd(b+1);
   dtHist[b]->Draw();
   sprintf(histname, "chan_%d_%d", b/8, b%8);
   dtHist[b]->SetTitle(histname);
   dtHist[b]->GetXaxis()->SetLabelSize(0.08);
   dtHist[b]->GetYaxis()->SetLabelSize(0.08);
   dCalRecoDelays = calRecoDelays[b/8] - calRecoDelays[b%8];
   if(dCalRecoDelays<0) dCalRecoDelays=settings->maxPaddedSample * settings->wInt_V + dCalRecoDelays;
   dtLine[b] = new TLine(dCalRecoDelays, 0, dCalRecoDelays, dtHist[b]->GetMaximum() * 1.2);
   dtLine[b]->SetLineColor(kBlue);
   dtLine[b]->SetLineStyle(7);
   dtLine[b]->SetLineWidth(2);
   dtLine[b]->Draw("same");
}
